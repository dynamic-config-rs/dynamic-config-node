...
**Verify a runtime bump like a change, because it is one.** A napi or
store-client bump gets `just check` — both addons and both suites — plus
`cargo deny check`. And the addon that ships is the one CI builds on five
runners: a bump that only builds here has been proven on one platform.

**Changelog:** a pin that reaches the addon goes under the package's
`### Security` — and here the scope is *not* only this repository's CI: an
addon is a built artefact, so whatever it was compiled against is what a
user installs.
